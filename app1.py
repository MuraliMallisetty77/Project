...
def register():
    ...
    if request.method == 'POST':
        ...
        if pwd == cpwd:
            ...
            sender_address = 'muralimallisetty2002@gmail.com'
            sender_pass = 'bhytfzwwjcproeij'
...
def login():
    if request.method == 'POST':
        ...
        if opt == '1':
            ...
            sql = "select * from user_registration where uname='%s' and pwd='%s' " % (username, password1)
...
def textback():
    if request.method == 'POST':
        ...
        for ss in range(0, int(lenn)):
            ...
            sss = "myans" + str(ss)
            ca = "currans" + str(ss)
            print(request.form[sss])

            ca1 = request.form[ca]
            sss1 = request.form[sss]

            sql = "insert into results(sid,sname,semail,ename,edate,ca,ua,status)values('" + str(sid) + "','" + str(
                name) + "','" + str(email) + "','" + str(s1) + "','" + str(s2) + "','" + str(ca1) + "','" + str(
                sss1) + "','" + str(val_data) + "')"

            print(sql)
            cursor.execute(sql)
            mydb.commit()
        sql1 = "insert into finalresults(sid,semail,ename,edate,ca,ua,status) values ('" + str(sid) + "','" + str(
            email) + "','" + str(s1) + "','" + str(s2) + "',(select count(*) from results where ca=ua and sid='" + str(
            sid) + "' and ename='" + str(s1) + "' and edate='" + str(
            s2) + "') ,(select count(*) from results where sid='" + str(sid) + "' and ename='" + str(
            s1) + "' and edate='" + str(s2) + "'),'" + str(val_data) + "' )"
        print(sql1)
        cursor.execute(sql1)
        mydb.commit()
...
def reply_mail(s=0, s1="", s2="",s3="",s4=""):
    if s4=="Fake":
        ...
        message['From'] = sender_address
        message['To'] = receiver_address
        message['Subject'] = 'Real time face recognition for library check in and check out system'
        message.attach(MIMEText(mail_content, 'plain'))
        ses = smtplib.SMTP('smtp.gmail.com', 587)
        ses.starttls()
        ses.login(sender_address, sender_pass)
